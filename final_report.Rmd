...
spv %>%
  ...
  ggplot() + geom_point(aes(value, practice, color = country_cluster),
                      cex = 2.3, alpha = 0.65) +
  scale_color_manual(values = colors, name = 'Country Cluster') +
  labs(title = 'Practices v. Values by Cultural Dimension and Country Cluster',
...
kable(slr_results, col.names = c('Cultural Dimension', 'Coefficient Value',
  ...
  kable_styling(latex_options = "striped") %>%
  row_spec(which(slr_results$p_value < alpha), background = '#E5F5E0')
```
